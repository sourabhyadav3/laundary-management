        ...
        data = (drivers || [])
          ...
          .map(d => {
            ...
            const driverAreas = rawAreas
              ...
              .filter(a => a && a !== '...' && a !== '…');
            const activePickups = pickups.filter(p => p.assignedStaff === d.driverName && p.status !== 'Completed').length;
            const activeDeliveries = deliveries.filter(del => del.assignedStaff === d.driverName && del.status !== 'Delivered' && del.status !== 'Failed').length;
